...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  Future<void> _calculateDynamicFee() async {
    if (_selectedLat != null && _selectedLng != null) {
      ...
      final response = await Get.find<OrderController>().getShippingFee(
        _selectedLat!,
        _selectedLng!,
        branchId,
      );
      ...
      if (response != null) {
        ...
        if (isSuccess) {
          ...
          if (isOutOfZone && fee != null) {
            bool? confirm = await Get.dialog<bool>(
              AlertDialog(
                ...
                content: Text(
                  'Estás fuera de nuestra zona base. Podemos realizar la entrega con cobro extra por distancia. El costo total de envío será de \$${fee.toStringAsFixed(2)}. ¿Deseas continuar?',
                ),
                actions: [
                  TextButton(
            ...
            AppSnackbar.success(
              'Tarifa calculada',
              'Costo de envío: \$${fee.toStringAsFixed(2)}',
            );
          } else {
            setState(() => _dynamicDeliveryFee = null);
          }
          ...
          if (isOutOfZone) {
            Get.dialog(
              AlertDialog(
                ...
                content: Text(
                  message.isNotEmpty
                      ? message
                      : 'El restaurante no tiene cobertura para tu ubicación.',
                ),
                actions: [
                  ElevatedButton(
                    onPressed: () => Get.back(),
            ...
            AppSnackbar.warning(
              'Error',
              message.isNotEmpty ? message : 'No se pudo calcular la tarifa',
            );
          }
          setState(() => _dynamicDeliveryFee = null);
        }
              ...
              children: [
                ...
                if (orderController.addressList.isNotEmpty) ...[
                  ...
                  ...orderController.addressList.map((addr) {
                    bool isSelected =
                        orderController.selectedAddress?.id == addr.id;
                    return GestureDetector(
                      onTap: () {
                        orderController.selectAddress(addr);
                        _addressController.text = addr.address ?? '';
                        if (addr.contactPersonName != null &&
                            addr.contactPersonName!.isNotEmpty) {
                          _contactNameController.text = addr.contactPersonName!;
                        }
                        if (addr.contactPersonNumber != null &&
                            addr.contactPersonNumber!.isNotEmpty) {
                          _contactPhoneController.text =
                              addr.contactPersonNumber!;
                        }
                        ...
                        child: IconButton(
                          onPressed: () async {
                            final LatLng? result = await Get.to(
                              () => const MapPinPickerView(
                                initialLat: 13.68935,
                                initialLng: -89.18718,
                              ),
                            ); // Default El Salvador or similar
                            if (result != null) {
                              setState(() {
                              ...
                              try {
                                ...
                                if (placemarks.isNotEmpty) {
                                  ...
                                  if (locality != null) {
                                    ...
                                    final matchedZone = zoneCtrl.zoneList
                                        .firstWhereOrNull(
                                          (z) =>
                                              locality.toLowerCase().contains(
                                                z.name.toLowerCase(),
                                              ) ||
                                    ...
                                    if (matchedZone != null) {
                                      ...
                                      AppSnackbar.info(
                                        'Zona detectada',
                                        'Se ha seleccionado la zona: ${matchedZone.name}',
                                      );
                                    } else {
                              ...
                              onPressed: () async {
                                bool? confirm = await Get.dialog<bool>(
                                  AlertDialog(
                                    ...
                                    content: const Text(
                                      '¿Estás seguro de que deseas eliminar esta tarjeta guardada?',
                                    ),
                                    actions: [
                                      TextButton(
                                        onPressed: () =>
                    ...
                    child: TextButton.icon(
                      onPressed: () {
                        setState(() {
                          ...
                          if (orderController.cardList.isNotEmpty) {
                            orderController.selectCard(
                              orderController.cardList.first,
                            );
                          }
                        });
                      },
          ...
          builder: (zoneController) {
            return GetBuilder<CouponController>(
              builder: (couponController) {
                ...
                final selectedZone = zoneController.zoneList.firstWhereOrNull(
                  (z) => z.id == zoneController.selectedZoneId,
                );
                ...
                if (couponController.hasCouponApplied &&
                    couponController.appliedCoupon?.coupon?.type ==
                        'free_delivery') {
                        ...
                        children: [
                          ...
                          ...cartList.map((item) {
                            ...
                            if (item.variantId != null &&
                              ...
                              final variant = item.product!.variants!
                                  .firstWhereOrNull(
                              ...
                              if (variant != null) {
                                ...
                                variantPrice =
                                    variant.priceModifier?.toDouble() ?? 0.0;
                              }
                            }
                            ...
                            if (item.extras != null &&
                                ...
                                item.product?.extras != null) {
                              final uniqueExtras = item.extras!.toSet();
                              for (var extraId in uniqueExtras) {
                                int count = item.extras!
                                    .where((e) => e == extraId)
                                    .length;
                                final extra = item.product!.extras!
                                    .firstWhereOrNull((e) => e.id == extraId);
                                if (extra != null) {
                                  double extraTotal =
                                      (extra.price?.toDouble() ?? 0.0) * count;
                                  extraWidgets.add(
                                    Padding(
                                ...
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        '\$${(basePrice * qty).toStringAsFixed(2)}',
                                        style: const TextStyle(
                                          fontWeight: FontWeight.w500,
                                        ),
                                      ),
                                    ...
                                    Padding(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            variantPrice >= 0
                                                ? "+\$${(variantPrice * qty).toStringAsFixed(2)}"
                                                : "-\$${(variantPrice.abs() * qty).toStringAsFixed(2)}",
                                            style: TextStyle(
                                              fontSize: 12,
                                              color: Colors.grey.shade600,
                                            ),
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    Text(
                                      'Cupón (${couponController.appliedCouponCode})',
                                      style: TextStyle(
                                        fontSize: 14,
                        ...
                        children: [
                          ...
                          if (couponController.hasCouponApplied) ...[
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          couponController.appliedCouponCode ??
                                              '',
                                          style: TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 14,
                            ...
                            if (couponController.userCoupons.isNotEmpty) ...[
                              ...
                              GestureDetector(
                                onTap: () =>
                                    _showUserCouponsSheet(couponController),
                                child: Container(
                                  padding: const EdgeInsets.symmetric(
                                    vertical: 10,
                                    horizontal: 14,
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(width: 8),
                                      Text(
                                        'Mis Cupones (${couponController.userCoupons.length})',
                                        style: TextStyle(
                                          color: AppColors.mainColor,
                                          fontWeight: FontWeight.w600,
                                          fontSize: 13,
                        ...
                        children: [
                          ...
                          Text(
                            _paymentMethod == 'cash_on_delivery'
                                ...
                                : (Get.find<OrderController>().selectedCard !=
                                          null
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (coupon.minOrderAmount != null &&
                                  ...
                                  Text(
                                    'Mínimo: \$${coupon.minOrderAmount!.toStringAsFixed(2)}',
                                    style: TextStyle(
                                      fontSize: 11,
    ...
    if (_currentStep == 0) {
      ...
      if (_addressController.text.isEmpty) {
        ...
        if (selected == null) {
          AppSnackbar.warning(
            'Dirección requerida',
            'Selecciona o ingresa una dirección de entrega',
          );
          return;
        }
      } else {
        ...
        if (_selectedLat == null || _selectedLng == null) {
          AppSnackbar.warning(
            'Ubicación requerida',
            'Usa el botón de mapa para seleccionar la ubicación exacta de entrega',
          );
          return;
        }
      }
          ...
          if (_expiryController.text.length < 5) {
            ...
            if (parts.length == 2) {
              ...
              if (year == null || year < currentYear) {
                AppSnackbar.warning(
                  'Tarjeta expirada',
                  'La tarjeta ya venció (año inválido)',
                );
                return;
              }
              ...
              if (year == currentYear && month < currentMonth) {
                AppSnackbar.warning(
                  'Tarjeta expirada',
                  'La tarjeta venció en el mes $month/$year',
                );
                return;
              }
            } else {
  ...
  Future<void> _placeOrder() async {
    ...
    if (orderController.selectedAddress != null &&
        orderController.selectedAddress!.id != null) {
      ...
      AddressModel newAddress = AddressModel(
        ...
        references: _referenceController.text.isNotEmpty
